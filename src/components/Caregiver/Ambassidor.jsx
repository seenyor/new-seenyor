...
const Ambassidor = () => {
  ...
  return (
    <section className="font-poppins w-full flex flex-col items-center justify-center py-5">
      <div className="container max-w-[1672px] flex flex-col gap-[80px] justify-center items-center">
        <h1 className="text-10 text-[#59B58A] font-bold leading-[176%]">
        ...
        <div className="flex gap-[60px] flex-wrap justify-center">
          {cards.map((item, index) => (
            <div
              key={index}
              className="flex flex-col gap-[10px] w-[505px] shadow-md rounded-[10px] p-[30px] justify-center items-center"
            >
              <item.icn />
              <h1 className="text-[24px] font-semibold">{item.title}</h1>
              <p className="text-[20px] font-medium text-center text-[#545454]">
                {item.text}
              </p>
            </div>
          ))}
